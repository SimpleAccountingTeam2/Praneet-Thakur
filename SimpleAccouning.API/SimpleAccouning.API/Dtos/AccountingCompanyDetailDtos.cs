using System;
using System.Collections.Generic;
using System.Linq;
using System.Web;

namespace SimpleAccouning.API.Dtos
{
    /// <summary>
    /// For mapping of  AccountingCompanyDetail to this call named: AccountingCompanyDetailDtos 
    /// </summary>
    public class AccountingCompanyDetailDtos
    {

        public int CompanyId { get; set; }
        public string CompanyName { get; set; }
        public int CompanyRegno { get; set; }
        public string CompanyTelephone { get; set; }
        // public decimal CompanyTax { get; set; }
        public string CompanyEmail { get; set; }
        public string FinancialYrStartDate { get; set; }
        //public string FinancialYrStateDate { get; set; }
        public byte[] CompanyLogo { get; set; }
        public string SoftwareSerialNo { get; set; }
        public string BilltoLine1 { get; set; }
        public string BilltoLine2 { get; set; }
        public string BilltoCity { get; set; }
        public string BilltoState { get; set; }
        public string BilltoCountry { get; set; }
        public string BilltoPostalCode { get; set; }
        public string ShiptoLine1 { get; set; }
        public string ShiptoLine2 { get; set; }
        public string ShiptoCity { get; set; }
        public string ShiptoState { get; set; }
        public string ShiptoCountry { get; set; }
        public string ShiptoPostalCode { get; set; }
    }
}